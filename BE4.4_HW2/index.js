...
async function deleteByid(id){
   try {
    const deleteRes =  await Models.findByIdAndDelete(id)
...
app.delete('/hotels/:hotelId' ,async (req,res)=>{
  try {
       const deleteRes = await deleteByid(req.params.hotelId)
    if(deleteRes){
...
async function updateData(dataId ,dataToUpdate ){
  try{
    const updateData = await  Models.findByIdAndUpdate(dataId ,dataToUpdate , {new :true})
      return updateData
  }catch(error){
     console.log("error in updateing data", error)

  }

}

app.post('/hotels/:updateId' ,async (req,res)=>{
  try {
    const updatedData = await  updateData(req.params.updateId , req.body)
    if(updatedData){
